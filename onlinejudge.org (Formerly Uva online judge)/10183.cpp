...
int main() {
    ...
    bigint f1(1),f2(2),f3(0),zero(0);

    nums.emplace_back(zero);
    nums.emplace_back(f1);
    nums.emplace_back(f2);
    while(f3.a.size()<=100) {
        f3= f1+f2;
        nums.emplace_back(f3);
        f1=f2;
        f2=f3;
    }
    //for(int i=1;i<=100;++i)nums[i].print();
    while(1) {
        string a,b;
        cin>>a;cin>>b;
        bigint x(a),y(b);
        if(a=="0" && b== "0") break;
        long long st=1,dr=nums.size();
        long long howManyLeft=0,howManyRight=0;
        while (st<=dr) {
            int mid=(st+dr)/2;
            if(nums[mid]<x) {
                howManyLeft=mid;
